...
float ppg_meanHrStressZ = NAN, ppg_sdnnStressZ = NAN, ppg_rmssdStressZ = NAN;
...
void ppg_updateStressScore(String mode) {
  ...
  if (ppg_baselineHRCount < 5) return; 

  ppg_meanHrStressZ = sys_computeZScore(ppg_meanHR, ppg_baselineMeanHRMean, ppg_baselineMeanHRStd, 3.0);
  ...
  float zSum = 0; int zCount = 0;
  if (!isnan(ppg_meanHrStressZ)) { zSum += ppg_meanHrStressZ; zCount++; }
...
void sys_updateCombinedStateAndPrint(unsigned long elapsedMs, String mode) {
  ...
  sys_ppgReadyForBle = labelMode && (ppg_validWindowFlag == 1) && !isnan(ppg_meanHrStressZ);
  ...
  if (sys_ppgReadyForBle) { addValidZ(ppg_meanHrStressZ); addValidZ(ppg_sdnnStressZ); addValidZ(ppg_rmssdStressZ); }
  ...
  if (ENABLE_SERIAL_CSV) {
    if (!sys_csvHeaderPrinted) {
      Serial.println("timestamp,mode,raw_gsr,ir_raw,filtered_ppg,ppg_mean_hr,ppg_sdnn,ppg_rmssd,gsr_scl,gsr_scr_amp,gsr_scr_freq,final_ppg_stress_z,final_gsr_stress_z,valid_feature_count,integrated_stress_z,integrated_state");
